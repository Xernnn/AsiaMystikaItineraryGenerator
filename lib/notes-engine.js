...
export function generateNotes(state, briefRows) {
  ...
  const allSelections = flattenSelections(state);
  const hasTriple = allSelections.some(sel => (sel?.rooms3pax || 0) > 0);
  if (hasTriple) {
  ...
  if (allSelections.some(sel => (sel?.focRooms || 0) > 0)) {
...
/**
 * Gather every `selection` object from `state.step4.selections`
 * — shape is `{ [stayId]: { [tier]: selection } }`.
 * The old code walked `stay.selections` which never existed.
 */
function flattenSelections(state) {
  const out = [];
  const byStay = state?.step4?.selections || {};
  for (const perTier of Object.values(byStay)) {
    if (!perTier) continue;
    for (const sel of Object.values(perTier)) {
      if (sel) out.push(sel);
    }
  }
  return out;
...
function anySurchargeSelected(state) {
  for (const sel of flattenSelections(state)) {
    if (sel.earlyCheckinDay) return true;
    if (sel.upgradeRoomTypeId) return true;
  }
  return false;
}
