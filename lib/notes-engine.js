...
export function generateIncludes(state, briefRows) {
  ...
  if (state.step2?.sapaZoneCar === 'split') {
    items.push('Sapa transfer vehicle combination: 1 × 29-seater + 1 × 16-seater (pax > 29)');
  } else if (state.step2?.sapaZoneCar === '29s') {
    items.push('Sapa transfer vehicle: 29-seater Private Car');
  } else if (state.step2?.sapaZoneCar === '16s') {
    items.push('Sapa transfer vehicle: 16-seater Private Car');
  }
  if (state.step2?.pqRachVemCar === 'split') {
    items.push('Phu Quoc (Rach Vem) vehicle combination: 1 × 29-seater + 1 × 16-seater (pax > 29)');
  }
