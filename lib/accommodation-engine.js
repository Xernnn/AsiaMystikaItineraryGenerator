...
export function buildStayBreakdown({ stay, sel, hotel, state, groupType, dateFrom }) {
  ...
  if (sel.earlyCheckinDay) {
    ...
    // One ECI charge per paid room on that day.
    const amt = perRoom * Math.max(1, paidRooms);
    if (amt > 0) {
      rows.push({
        kind: 'eci',
        label: 'Early check-in',
        detail: `Day ${sel.earlyCheckinDay}: ${formatMoney(perRoom, currency)} × ${Math.max(1, paidRooms)} room${paidRooms === 1 ? '' : 's'}`,
        amount: amt,
      });
    }
  }
  ...
  if (upgradeRoomType) {
    ...
    if (diff > 0) {
      const amt = diff * Math.max(1, nights) * Math.max(1, paidRooms);
      rows.push({
        kind: 'upgrade',
        label: `Upgrade → ${upgradeRoomType.name}`,
        detail: `(${formatMoney(upRate, currency)} − ${formatMoney(ratePerRoom, currency)}) × ${nights || 0} night${nights === 1 ? '' : 's'} × ${Math.max(1, paidRooms)} room${paidRooms === 1 ? '' : 's'}`,
        amount: amt,
      });
    }
  }
